...
import scalax.file.{ FileOps, Path, NotFileException }
  ...
  def main(args: Array[String]) {
    //p373(args(0).toInt)
//    val r = List(8453,9686) map { x => p373Naive(x) - p373T2(x) }
//    println(" r = " + r)
    //messages
    //p373T(args(0).toInt)
/*    val m1 = p373T(args(0).toInt)
    val m2 = p373Naive(args(0).toInt)
    val diff = (m2.toSet -- m1.toSet)
    val keys = diff.map{_._1}.toList.sorted
    println(keys.map{r => 
      val m2r = m2(r)		     
      val m1r = m1(r)
      val diffr = m1r -- m2r
      r + " diff = " + diffr.toList.sortBy(_(0)) + "\n\n" +  m2r.toList.sortBy(x=>x(0)) + "===== \n" + m1r.toList.sortBy(x=>x(0)) + "\n\n"})
  */    
    //no messages
    p373T2(args(0).toInt)
  ...
  def round(d: Double) = if (d.toString.contains("000000")) math.floor(d).toInt else math.ceil(d).toInt
...
*/ 
    ...
    while (i < n) {
       ...
       */ 
      val kcRight = lim/t(2)
      val nsr = 1L*t(2) * kcRight * (kcRight + 1)/2
      s = s + nsr 
      if (kcRight > 0) println("right triangle: " + List(t(0), t(1), t(2)).sorted + " nsr = " + nsr)
      abcL =  List(1.*t(0), 1.*t(1), 1.*t(2)).sorted +: abcL 
      //List.range(1, kcRight+1) foreach {k => mR = mR.updated(k*t(2), mR(k*t(2)) ++ Set(List(2*k*t(0), 2*k*t(1), 2*k*t(2)).sorted))}
      ...
      val nsIso = 1L*t(2) * t(2) * kcIsosceles * (kcIsosceles + 1)
      s = s + nsIso
      if (kcIsosceles >= 1) {
	visitedIsoBases = visitedIsoBases + 2.*t(0)*t(1)
	visitedIsoAux(iso) = Set((t(1),t(2)),(t(0),t(2)))
	abcL = List(1.*t(2)*t(0), 1.*t(2)*t(0), 2.*t(0)*t(1)).sorted +: List(1.*t(2)*t(1), 1.*t(2)*t(1), 2.*t(1)*t(0)).sorted +: abcL
	/*List.range(1, kcIsosceles+1) foreach {
	  k => 
	    mR = mR.updated(k*t(2)*t(2), mR(k*t(2)*t(2)) ++ Set(List(2*k*t(2)*t(0), 2*k*t(2)*t(0), 4*k*t(0)*t(1)).sorted, List(2*k*t(2)*t(1), 2*k*t(2)*t(1), 4*k*t(1)*t(0)).sorted))
			  }
*/
	...
	val kc1 = lim / l1
        val ns1 = 1L * l1 * kc1 * (kc1 + 1) / 2
	val qr2 = q1q2 / d2
	val fr2 = factor2 / d2
	val l2 = lcmL(mq1q2, qr2)
	//val l2 = lcm(t(2), lcm(tp(2), qr2))
	val kc2 = lim / l2
	val ns2 = 1L*l2 * kc2 * (kc2 + 1) / 2
	val a1 = 1.*p1/q1
	val a2 = 1.*r1/q1
	val b1 = 1.*p2/q2
	val b2 = 1.*r2/q2
	...
	if(kc1 > 0 || kc2 > 0) {	  
	  ...
	  val notIsoCII2 = (a2 != b1 && b1 != c2 && c2 != a2)
		...
		//List.range(1, kc1+1) foreach { k => mR = mR.updated(k*l1, mR(k*l1) ++ Set(abc.sorted.map{round(_)*2*k})) }
		...
		s = s + ns2
		abcL = abc.sorted +: abcL
		//List.range(1, kc2+1) foreach { k => mR = mR.updated(k*l2, mR(k*l2) ++ Set(abc.sorted.map{round(_)*2*k})) }
		...
		if ( d1 > 1 && kc1 > 0 && 
		    ( notIsoCI1 && ( (visited(k)(j) != null && visited(k)(j)((p1,q1,l1))) || (visited(i)(k) != null && visited(i)(k)((p2,q2,l1))) ) ) ) repeatedI1 = true
		if ( d1 > 1 && kc1 > 0 && 
		    ( notIsoCI2 && ( (visited(k)(j) != null && visited(k)(j)((r1,q1,l1))) || (visited(i)(k) != null && visited(i)(k)((r2,q2,l1))) ) ) ) repeatedI2 = true
		if ( d2 > 1 && kc2 > 0 && 
		    ( notIsoCII1 && ( (visited(k)(j) != null && visited(k)(j)((p1,q1,l2))) || (visited(i)(k) != null && visited(i)(k)((r2,q2,l2))) ) ) ) {
		      ii2 = "k = " + k + " j = " + j + " i = " + i + " (p1,q1) = " + (p1,q1) + " (r2, q2) = " + (r2,q2)
		      repeatedII1 = true
		    }
		if ( d2 > 1 && kc2 > 0 && 
		    ( notIsoCII2 && ( (visited(k)(j) != null && visited(k)(j)((r1,q1,l2))) || (visited(i)(k) != null && visited(i)(k)((p2,q2,l2))) ) ) ) repeatedII2 = true
		k = k + 1
	      }
	      if (k == i) {		
		...
		while (k < j && (repeatedI1 == false || repeatedI2 == false || repeatedII1 == false || repeatedII2 == false)) {
		  if (visited(i)(k) != null) {
		    ...
		    if ((d2 > 1 && kc2 > 0 && !repeatedII1 && visited(i)(k)((r2,q2,l2))) ) {
		      repeatedII1 = true
      		      ii2 = "k = " + k + " i = " + i + " (r2, q2) = " + (r2,q2)
		    }
		...
		if (d2 > 1 && kc2 > 0 && !repeatedII1) {
		  ...
		  abc = List(l2*a1,l2*b2,l2*c2)
		  if(notIsoCII1) {
		  if(abc.max <= lim && kc2 > 0 && d2 > 1) {
		    nabcL = abc.sorted +: nabcL
		    println("abcrii1 = " + abc + " t = " + t + " tp = " + tp + " repeatedII1 = " + repeatedII1 + " info = " + ii2 + " at (i,j) = " + (i,j) + " not added")		
		  }
...
/*    val x = for(i <- List.range(0,n); j <- List.range(i+1, n); if (visited(i)(j) != null && visited(i)(j)((56,65,845)) ) ) yield (i,j)
    println("x = " + x)
    val visitedL = (visited.toList.zipWithIndex map {xi => xi._1.toList.zipWithIndex map {xii => (xii._1, xi._2, xii._2)}} flatten).filter(_._1!=null)
    println("visited = " + visitedL.take(50))
    */ 
    /*val mRF = mR.filter(_._2.size!=0)
    val s1 = mRF.foldLeft(0)((r, c) => ( r + c._1*c._2.size))
    println("s1 = " + s1)
    println("mRF(25) = " + mRF(25))
    */
  ...
  while (radius <= lim) {
	...
	while (j < n) {
	  ...
	  if ( (radius % tp(2)) == 0 ) {
	    ...
	    val factor1 = t(0)*tp(1) + t(1)*tp(0)
	    val cond1 = (1L*radius*factor1) % q1q2 == 0
	    if (cond1) {
	      ...
	      val c = (2L*radius*factor1/q1q2).toInt
	    ...
	    val factor2 = t(1)*tp(1) + t(0)*tp(0)
	    val cond2 = (1L*radius*factor2) % q1q2 == 0
	    if (cond2) {
	      val c = (2L*radius*factor2/q1q2).toInt
