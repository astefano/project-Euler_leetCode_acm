def brickwall(w,h):
    ...
    def gen_layers(w):
        ...
        return [(layer + [2]) for layer in gen_layers(w-2)] + [(layer + [3]) for layer in gen_layers(w-3)]
    ...
    def gen_conflict_mat(layers, nlayers, w):
        ...
        for i in range(nlayers):
            mat.append([j for j in range(nlayers) if intpos[i].isdisjoint(intpos[j])])
        #print 'Mat', mat    
        return mat
    layers = gen_layers(w)
    #print 'Layers', layers
