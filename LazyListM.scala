...
trait LazyConsable[A] {
  def mconsl (rest : => LazyList[A]) : LazyList[A]
}
...
object LazyListImplicit {
  implicit def lazyConsable[A] (first : A) : LazyConsable[A] = new LazyConsable[A] {
    def mconsl (rest : => LazyList[A]) = new LazyCons(first,rest)
  }
}
...
object TestLazyList {
  ...
  def main (args : Array[String]) {
    ...
    def intsFrom(n : Int) : LazyList[Int] = n mconsl intsFrom(n+1)
    ...
    val safeLazy : LazyList[Int] = 3 mconsl ({ println("4 got eval'd") ;  4 }  mconsl LazyNil)
    // Prints nothing.
    ...
    val nukeMe : LazyList[Int] = 1  mconsl (2 mconsl (throw new Exception("Boom!")))
